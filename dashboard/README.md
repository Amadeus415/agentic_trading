...
Read-only Next.js UI over the paper-fund SQLite ledger (`state/edgecraft-aggressive.db`).
...
Pages read the SQLite ledger directly (`src/lib/fund.ts`). There is no JSON API and no write path.
