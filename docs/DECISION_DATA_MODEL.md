...
immutable `edgecraft.decision-audit.v2` packet before Edgecraft creates a trade
...
```text
mandate + risk policy + research + market/context + decision memory
                              │
                              ▼
...
| `decision_memory` | A content-hashed, bounded record of prior thesis mechanisms, horizons, falsifiers, allocations, next-period benchmark-relative outcomes, and aggregate evaluation shown to the model. |
...
- action, confidence, hypothesis, causal thesis mechanism, expected horizon,
  falsifiers, risks, and alternatives considered;
- prior run IDs when historical decisions materially influenced the new one;
