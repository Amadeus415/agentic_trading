...
# Staleness: mandate cadence + documented grace. Weekly default is 7d + 1d grace.
# Market-day mandates tighten the threshold to 1d + 1d grace when any are registered.
DEFAULT_SUCCESS_CADENCE_SECONDS = 7 * 24 * 3600
MARKET_DAY_SUCCESS_CADENCE_SECONDS = 24 * 3600
SUCCESS_STALE_GRACE_SECONDS = 24 * 3600
...
def success_staleness_threshold_seconds(ledger: AuditLedger) -> int:
    """Cadence + grace used to degrade autonomy-health when last success is too old."""
    try:
        mandates = ledger.list_mandates()
    except Exception:
        mandates = []
    cadence = DEFAULT_SUCCESS_CADENCE_SECONDS
    if any(getattr(m, "cycle_frequency", None) == "market_day" for m in mandates):
        cadence = MARKET_DAY_SUCCESS_CADENCE_SECONDS
    return cadence + SUCCESS_STALE_GRACE_SECONDS
...
def autonomy_health(ledger: AuditLedger) -> dict[str, Any]:
    ...
    stale_after_seconds = success_staleness_threshold_seconds(ledger)
    reasons = []
    ...
    if last_success is None:
        reasons.append("no successful autonomous run recorded")
    elif last_success_age_seconds is not None and last_success_age_seconds > stale_after_seconds:
        reasons.append(
            "last successful run is stale "
            f"(age_seconds={last_success_age_seconds}, threshold_seconds={stale_after_seconds})"
        )
    ...
    return {
        ...
        "success_stale_after_seconds": stale_after_seconds,
        "snapshot": snapshot,
    }
