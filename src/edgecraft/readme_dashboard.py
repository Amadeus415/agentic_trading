...
def build_console_markdown(
    ...
    *,
    ...
    """Render a deliberately aggregate-only public operations snapshot."""
    ...
    reason_line = (
        " · ".join(_clean_public_text(str(reason)) for reason in reasons)
        if reasons
        else "No control-plane warnings."
    )
    ...
    return "\n".join(
        [
            ...
            f"| Fills recorded | **{filled}** | Filled lifecycle events recorded and reconciled |",
            "",
            "<sub>Generated from privacy-safe aggregate ledger fields. Account identifiers, symbols, "
            "positions, order sizes, broker payloads, and model prompts are never written here. A test, "
            "proposal, or permit is not counted as a fill.</sub>",
            END_MARKER,
        ]
    )
